...
type expression = 
  ...
  | Try_exp of expression * var * expression
  | Raise_exp of expression
...
type continuation =
  ...
  | Try_cont of var*expression*env*continuation
  | Raise1_cont of continuation
...
let rec apply_cont (c: continuation) (v: exp_value) : final_answer =
  ...
  | Try_cont(var, handler_exp, saved_env, saved_cont) -> apply_cont saved_cont v
  | Raise1_cont(saved_cont) -> apply_handler v saved_cont
...
and value_of_k (exp:expression) (environment:env ) (c:continuation) : final_answer =
  ...
  | Try_exp(exp1, var, handler_exp) -> value_of_k exp1 environment (Try_cont(var, handler_exp, environment, c))
  | Raise_exp(exp1) -> value_of_k exp1 environment (Raise1_cont c)
                         
                         
and apply_handler (v: exp_value) (c: continuation): final_answer =
  match c with
  | Try_cont(var, handler_exp, saved_env, saved_cont) -> value_of_k handler_exp (extend_env var v saved_env) saved_cont
  | End_cont -> raise Invalid 
  | Diff1_cont(exp2, saved_env, saved_cont) -> apply_handler v saved_cont
  | Diff2_cont(val1, saved_cont) -> apply_handler v saved_cont
