...
type bounce = BounceVal of exp_value | UnitCont of (unit -> bounce)
...
let rec apply_cont (c: continuation) (v: exp_value) : bounce =
  match c with 
  | End_cont -> (Printf.printf "End of computation." ) ; BounceVal v
...
and value_of_k (exp:expression) (environment:env ) (c:continuation) : bounce =
  match exp with
...
and apply_procedure_k (p:procedure) (value:exp_value) (c:continuation) : bounce =
  value_of_k p.body (extend_env p.var value  p.saved_env ) c 
                               
let rec trampoline (b:bounce) : final_answer =
  match b with 
  | BounceVal(b) -> expval_to_finalanswer b
  | _ -> trampoline b 
...
let value_of_program (p:program) : final_answer = 
  ...
  | Expression(exp) -> let init_env = empty_env() in
      ...
      (trampoline (value_of_k exp init_env end_c))
