...
type store = {mutable store : exp_value option array; mutable size: int}


let empty_store (size: int) : store =
	match size with
	| 0 -> let (listOfReferences: exp_value option array) = [||] in
		{store = listOfReferences; size = 0;}
	| _ -> let (listOfReferences: exp_value option array) = Array.make size None in
		{store = listOfReferences; size = 0;}

(* initialize store *)
let global_store = ref (empty_store 0)
...
let newref (value:exp_value)  : reference = 
	let new_index = global_store.size in
	let store_length = global_store.store.length in
		(if new_index >= store_length then let new_store = empty_store (store_length * 2) in 
			blit global_store.store 0 new_store 0 store_length; global_store.store <- new_store);
		global_store.store.(new_index) <- value;
		global_store.size <- (new_index + 1);
		new_index
...
let initialize_store (size:int) = 
	let s = empty_store size in
		global_store := s

let rec initial_store_size (exp:expression) (count: int): int = 
	match exp with
		| Newref_exp(exp1) -> initial_store_size exp1 (count + 1)
		| _(exp1) -> initial_store_size exp1 count

let value_of_program (p:program) : int = 
  match p with
  | Expression(exp) -> let store_size = initial_store_size exp 0 in 
  	let init_env = empty_env in
      initialize_store store_size; expval_to_num (value_of exp init_env)
